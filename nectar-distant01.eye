Eye.application :dev02 do
	...
	working_dir File.expand_path(File.join(File.dirname(__FILE__), %w[ processes ]))
        ...
        ## NOT USED -- distant one is used later
	process :redis do
	    daemonize true
...
#		stdall 'apps/camera-server.log'
		...
		start_command "java -jar -Xmx128m apps/camera-server.jar --driver OPENNI2 --device-id 0 --format rgb --output camera0 --stream --depth-camera -rh oj.lity.tech -rp 6389 "
        depend_on :redis
...
#        stdall   'apps/chilitags-server.log'
        ...
        start_command "apps/chilitags-server --input camera0 --output camera0:markers --camera-parameters camera0 --stream --redis-host oj.lity.tech --redis-port 6389"
        depend_on :camera
...
#        stdall   'apps/chilitags-server.log'
        ...
        start_command "apps/aruco-server --input camera0 --output camera0:markers --camera-parameters camera0 --stream  --redis-host oj.lity.tech --redis-port 6389"
        depend_on :camera
        check :memory, every:10, below: 200.megabytes
    end
...
#        stdall   'apps/chilitags-server.log'
        ...
        start_command "apps/artk-server --input camera0 --output camera0:markers --camera-parameters camera0 --stream --redis-host oj.lity.tech --redis-port 6389"
        depend_on :camera
        check :memory, every:10, below: 200.megabytes
    end
...
#		stdall 'apps/pose-server.log'
		start_command "java -jar -Xmx64m apps/pose-estimator.jar --input camera0 --output sheet:pose --marker-configuration apps/chili1.svg --camera-configuration apps/calibration-AstraS-rgb.yaml --stream -rh oj.lity.tech -rp 6389"
...
#        stdall 'apps/camera-test.log'
        ...
        start_command "java -jar -Xmx64m apps/camera-server-test.jar --input camera0 -rh oj.lity.tech -rp 6389"
        depend_on :camera
    end
